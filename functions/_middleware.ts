...
function openapiSpec(origin: string) {
  return {
    ...
    info: {
      ...
      version: "1.0.0",
      description:
        "Public web API exposing read-only agent surfaces: the agent catalog, sitemap, llms.txt, and per-page markdown alternates.",
    },
    servers: [{ url: origin }],
...
export const onRequest: PagesFunction = async (context) => {
  ...
  if (pathname === "/openapi.json" || pathname === "/openapi.yaml") {
    return new Response(JSON.stringify(openapiSpec(origin), null, 2), {
      headers: {
        "content-type": "application/json; charset=utf-8",
        "access-control-allow-origin": "*",
